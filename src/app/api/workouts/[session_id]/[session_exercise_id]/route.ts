...
export async function POST(request: Request,  { params }: { params: Promise<{ session_id: string, session_exercise_id: string }> }) {
  ...
  const { data, error: insertError } = await supabase.from("session_sets").insert({session_exercise_id: session_exercise_id, set_number: set_number, set_reps: 0, set_rest_time: 0}).select().single()
